...
function App() {
  ...
  if (route === '/admin' && isAdmin) return <Dashboard submissions={filtered} search={search} setSearch={setSearch} sort={sort} setSort={setSort} status={status} logout={logout} goUpload={() => navigate('/')} onDelete={async (item) => { if (!window.confirm(`Delete ${item.name}'s photo and message?`)) return; const result = await supabase.from('submissions').delete().eq('id', item.id); if (result.error) return setStatus({ type: 'error', message: result.error.message }); if (item.image_path) { const removal = await supabase.storage.from('photos').remove([item.image_path]); if (removal.error) return setStatus({ type: 'error', message: removal.error.message }) } setSubmissions((items) => items.filter((submission) => submission.id !== item.id)); setStatus({ type: 'success', message: 'Photo deleted.' }) }} />
  if (route === '/admin') { navigate('/admin/login'); return null }
  return <main className="shell"><header className="topbar"><span className="brand">Cute Photo Wall</span><nav className="top-actions"><a className="instagram-link" href="https://www.instagram.com/_krish_mankind/" target="_blank" rel="noreferrer">Instagram</a><button className="text-button" onClick={() => navigate('/admin/login')}>Admin</button></nav></header><section className="hero-copy"><p className="eyebrow">A tiny corner for big feelings</p><h1>Share a Cute Moment</h1><p>Upload a photo and leave a little message.</p></section><form className="form-card" onSubmit={submit}><label>Name<input value={form.name} maxLength="50" onChange={(e) => updateForm('name', e.target.value)} placeholder="Enter your name" /></label><label>Message<textarea value={form.message} maxLength="250" onChange={(e) => updateForm('message', e.target.value)} placeholder="Write something..." rows="4" /></label><div className="photo-field"><span className="field-label">Photo</span>{form.preview ? <div className="upload has-preview"><img src={form.preview} alt="Selected preview" /><span>{form.photo.name}</span><button type="button" className="replace" onClick={() => { updateForm('photo', null); updateForm('preview', '') }}>Replace photo</button></div> : <div className="photo-actions"><label className="photo-option" aria-label="Upload a photo"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => selectPhoto(e.target.files[0])} /><Upload className="photo-icon" strokeWidth={1.8} aria-hidden="true" /></label><label className="photo-option" aria-label="Take a photo"><input type="file" accept="image/jpeg,image/png,image/webp" capture="environment" onChange={(e) => selectPhoto(e.target.files[0])} /><Camera className="photo-icon" strokeWidth={1.8} aria-hidden="true" /></label></div>}</div>{status.message && <div className={`notice ${status.type}`}>{status.message}{status.message === '💕 Added Successfully!' && <button type="button" onClick={() => setStatus({ type: '', message: '' })}>Add Another</button>}</div>}<button className="primary" disabled={isSubmitting}>{isSubmitting ? 'Adding...' : 'Add to wall'} <span>→</span></button></form><footer>Made with a little extra sweetness <span>✦</span></footer></main>
}
...
function Dashboard({ submissions, search, setSearch, sort, setSort, status, logout, goUpload, onDelete }) { const today = submissions.filter((item) => new Date(item.created_at).toDateString() === new Date().toDateString()).length; return <main className="dashboard"><header className="dash-header"><div className="dash-title"><p className="eyebrow">Your private view</p><h1>Cute Photo Wall <span>— Admin Panel</span></h1></div><nav className="actions" aria-label="Admin navigation"><button type="button" className="text-button active" onClick={goUpload}>Upload page</button><a className="instagram-link" href="https://www.instagram.com/_krish_mankind/" target="_blank" rel="noreferrer">Instagram</a><button type="button" className="text-button" onClick={logout}>Logout</button></nav></header>{status.message && <div className={`toast ${status.type}`}>{status.message}</div>}<section className="stats"><div><small>Total submissions</small><strong>{submissions.length}</strong></div><div><small>Today's submissions</small><strong>{today}</strong></div><div><small>Latest submission</small><strong>{submissions[0] ? new Date(submissions[0].created_at).toLocaleDateString() : '—'}</strong></div></section><div className="gallery-tools"><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name..." /><select value={sort} onChange={(e) => setSort(e.target.value)}><option value="newest">Newest first</option><option value="oldest">Oldest first</option></select></div><section className="gallery">{submissions.length ? submissions.map((item) => <article className="submission-card" key={item.id}><a href={item.image_url} target="_blank" rel="noreferrer"><img src={item.image_url} alt={`Uploaded by ${item.name}`} /></a><div className="card-body"><strong>{item.name}</strong><p>{item.message}</p><small>{new Date(item.created_at).toLocaleString()}</small><button className="delete" onClick={() => onDelete(item)}>Delete photo</button></div></article>) : <div className="empty">No submissions yet. Your wall is waiting for its first sweet moment.</div>}</section></main> }
